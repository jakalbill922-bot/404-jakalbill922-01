...
class TrellisService:
    ...
    def generate(
        ...
        start = time.time()
        try:
            # V5: Higher voxel_threshold + MULTIDIFFUSION
            # - Objects with <35000 voxels will get +50% slat_steps (30 steps)
            # - Multidiffusion: all views combined at each step (better quality)
            outputs, num_voxels = self.pipeline.run_multi_image_with_voxel_count(
                ...
                mode="multidiffusion",  # V5: multidiffusion for better quality
            )
