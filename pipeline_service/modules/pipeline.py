...
possible_paths = [
    Path("./modules/feature_info.pt"),
    Path(__file__).parent / "modules/feature_info.pt",
    Path(__file__).parent.parent / "modules/feature_info.pt",
]
...
class GenerationPipeline:
    def __init__(self, settings: Settings = settings):
        ...
        for path in possible_paths:
            if path.exists():
                self.feature_info = torch.load(str(path))
                logger.info(f"Feature info loaded from {path}")
                break
